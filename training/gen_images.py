...
def parse_args(args=None):
    ...
    parser.add_argument("--gen_from_pca_model", type=str)
    parser.add_argument("--pca_override", type=str)
...
def gen_images(dataset_path, model_id, images_per_prompt, gen_from_pca_model, pca_override, overwrite_existing, **kwargs):
    model_files = os.listdir(dataset_path)
    random.shuffle(model_files)
    X = None
    sorted_keys = None
    
    if gen_from_pca_model is not None:
        leap = LM.load_from_checkpoint(gen_from_pca_model)
        leap.eval()
        if pca_override is not None:
            with open(pca_override, 'rb') as f:
                leap.pca = pickle.load(f)
    ...
    for model_file in tqdm(model_files, desc="Generating images..."):
        ...
        if gen_from_pca_model is not None:
            ...
            with open_safetensors(model_file_path, framework="pt") as f:
                ...
                # tensor = torch.tensor(leap.pca['scaler'].transform(tensor.unsqueeze(0).cpu().numpy()), dtype=torch.float32).to(tensor.device).squeeze(0)
                boosted_embed = leap.post_process(tensor)
                tmpdir = tempfile.TemporaryDirectory()
                model_file_path = os.path.join(tmpdir.name, "generated_tensors.safetensors")
                save_safetensors(boosted_embed, model_file_path, metadata_obj)            
                if os.path.exists(model_file_path):
                    generated_images_path = os.path.join(dataset_path, model_file, "images_generated_pca")
        else:
